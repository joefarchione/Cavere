...
let ``Topological sort orders accums by dependency`` () =
    ...
    let accums = Map.ofList [
        0, { Init = 0.05f.C; Body = 0.05f.C }
        1, { Init = 100.0f.C; Body = AccumRef 1 * Expr.exp (AccumRef 0 * 0.01f.C) }
    ]
...
let ``Surface2d generates interpolation code`` () =
    let m = model {
        let! sid = surface2d [| 0.0f; 1.0f |] [| 50.0f; 150.0f |]
                      (array2D [| [| 0.20f; 0.20f |]; [| 0.20f; 0.20f |] |]) 252
        let! z = normal
