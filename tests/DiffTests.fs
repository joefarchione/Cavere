...
let ``collectDiffVars finds DiffVar in simple expression`` () =
    let expr = Add(Dual(0, 100.0f, "0"), Const 1.0f)
...
let ``collectDiffVars finds multiple DiffVars`` () =
    let expr = Mul(Dual(0, 100.0f, "0"), Add(Dual(1, 0.2f, "1"), Const 1.0f))
    let vars = CompilerDiff.collectDiffVars expr
...
let ``collectDiffVars finds DiffVar through nested ops`` () =
    let expr = Exp(Sqrt(Add(Dual(3, 5.0f, "3"), Const 1.0f)))
    let vars = CompilerDiff.collectDiffVars expr
    Assert.Equal<int>(Set.singleton 3, vars)

[<Fact>]
let ``collectDiffVars finds DiffVar in Select`` () =
    let expr = Select(Gt(Dual(0, 1.0f, "0"), Const 0.0f), Dual(1, 2.0f, "1"), Const 0.0f)
    let vars = CompilerDiff.collectDiffVars expr
    Assert.Equal(2, vars.Count)
...
let ``collectModelDiffVars finds DiffVars in result`` () =
    let m = {
        Result = Add(Dual(0, 100.0f, "0"), Const 1.0f)
        Accums = Map.empty
        Surfaces = Map.empty
        Observers = []
...
let ``collectModelDiffVars finds DiffVars in accumulators`` () =
    let m = {
        Result = AccumRef 0
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0"); Body = AccumRef 0 * Dual(1, 0.05f, "1") }
        ]
        Surfaces = Map.empty
        Observers = []
        NormalCount = 0; UniformCount = 0; BatchSize = 0
...
let ``collectModelDiffVars finds DiffVars in observers`` () =
    let m = {
        ...
        Accums = Map.empty
        Surfaces = Map.empty
        Observers = [ { Name = "test"; Expr = Dual(2, 42.0f, "2"); SlotIndex = 0 } ]
        NormalCount = 0; UniformCount = 0; BatchSize = 0
    }
    let vars = CompilerDiff.collectModelDiffVars m
    Assert.Equal(1, vars.Length)
...
let ``collectModelDiffVars returns sorted unique indices`` () =
    let m = {
        Result = Add(Dual(2, 1.0f, "2"), Dual(0, 2.0f, "0"))
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0"); Body = AccumRef 0 + Dual(1, 3.0f, "1") }
        ]
        Surfaces = Map.empty
        Observers = []
        NormalCount = 0; UniformCount = 0; BatchSize = 0
...
let ``forwardDiff of DiffVar wrt itself is 1`` () =
    let d = CompilerDiff.forwardDiff (Dual(0, 5.0f, "0")) 0 id
    Assert.Equal(Const 1.0f, d)

[<Fact>]
let ``forwardDiff of DiffVar wrt other is 0`` () =
    let d = CompilerDiff.forwardDiff (Dual(0, 5.0f, "0")) 1 id
...
let ``forwardDiff of Add follows sum rule`` () =
    // d/dx (x + 3) = 1 + 0
    let expr = Add(Dual(0, 2.0f, "0"), Const 3.0f)
...
let ``forwardDiff of Floor is 0`` () =
    let d = CompilerDiff.forwardDiff (Floor(Dual(0, 1.5f, "0"))) 0 id
    Assert.Equal(Const 0.0f, d)
...
let ``transformDual adds derivative accumulators for one DiffVar`` () =
    ...
    let m = {
        Result = AccumRef 0
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0"); Body = AccumRef 0 * 1.01f.C }
        ]
        Surfaces = Map.empty
        Observers = []
        NormalCount = 0; UniformCount = 0; BatchSize = 0
...
let ``transformDual adds derivative accumulators for two DiffVars`` () =
    let m = {
        Result = AccumRef 0
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0"); Body = AccumRef 0 * Exp(Dual(1, 0.05f, "1")) }
        ]
        Surfaces = Map.empty
        Observers = []
        NormalCount = 0; UniformCount = 0; BatchSize = 0
...
let ``transformDual derivative accum init is differentiated`` () =
    ...
    let m = {
        Result = AccumRef 0
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0"); Body = AccumRef 0 }
        ]
        Surfaces = Map.empty
        Observers = []
        NormalCount = 0; UniformCount = 0; BatchSize = 0
...
let ``transformDual expanded model compiles`` () =
    let m = {
        Result = AccumRef 0
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0"); Body = AccumRef 0 * Exp(Dual(1, 0.001f, "1")) }
        ]
        Surfaces = Map.empty
        Observers = []
        NormalCount = 0; UniformCount = 0; BatchSize = 0
...
let ``transformDual with multiple accums creates correct derivative count`` () =
    let m = {
        ...
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0"); Body = AccumRef 0 * 1.01f.C }
            1, { Init = Const 1.0f; Body = AccumRef 1 * Exp(-Dual(1, 0.05f, "1") / 252.0f.C) }
        ]
        Surfaces = Map.empty
        Observers = []
        NormalCount = 0; UniformCount = 0; BatchSize = 0
...
let ``DiffVar emits as constant value in C# source`` () =
    ...
    let code = CompilerCommon.emitExpr layout (Dual(0, 3.14f, "0"))
...
let ``AD model with DiffVar compiles and produces source`` () =
    ...
    let m = {
        Result = AccumRef 0
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0")
                 Body = AccumRef 0 * Exp((Dual(1, 0.05f, "1") - 0.5f.C * Dual(2, 0.2f, "2") * Dual(2, 0.2f, "2")) / 252.0f.C
                                         + Dual(2, 0.2f, "2") * Sqrt(1.0f.C / 252.0f.C) * Normal 0) }
        ]
        Surfaces = Map.empty
        Observers = []
...
let ``AD model runs on CPU and produces finite results`` () =
    ...
    let m = {
        Result = AccumRef 0
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0"); Body = AccumRef 0 * 1.001f.C }
        ]
        Surfaces = Map.empty
        Observers = []
        NormalCount = 0; UniformCount = 0; BatchSize = 0
...
let ``partialDiffAccumRef of product applies product rule`` () =
    ...
    let pd = CompilerDiff.partialDiffAccumRef (Mul(AccumRef 0, Dual(1, 0.05f, "1"))) 0
    // = 1 * DiffVar(1, 0.05) + AccumRef(0) * 0
    Assert.Equal(Add(Mul(Const 1.0f, Dual(1, 0.05f, "1")), Mul(AccumRef 0, Const 0.0f)), pd)

[<Fact>]
let ``partialDiffDiffVar of DiffVar(target) is 1`` () =
    let pd = CompilerDiff.partialDiffDiffVar (Dual(0, 5.0f, "0")) 0
    Assert.Equal(Const 1.0f, pd)
...
let ``adjoint with stochastic model produces delta`` () =
    let m = {
        ...
        Accums = Map.ofList [
            0, { Init = Dual(0, 100.0f, "0")
                 Body = AccumRef 0 * Exp(Dual(1, 0.05f, "1") / 252.0f.C
                                         + Dual(2, 0.2f, "2") * Sqrt(1.0f.C / 252.0f.C) * Normal 0) }
        ]
        Surfaces = Map.empty
        Observers = []
...
let ``recommend returns Adjoint for many DiffVars`` () =
    ...
    let m = {
        Result = Dual(0, 1.0f, "0") + Dual(1, 2.0f, "1") + Dual(2, 3.0f, "2")
                 + Dual(3, 4.0f, "3") + Dual(4, 5.0f, "4") + Dual(5, 6.0f, "5")
        Accums = Map.ofList [
...
let ``HyperDual vomma: GBM call d²V/dvol² is non-zero`` () =
    // vol as HyperDual → should produce __deriv_vol and __deriv2_vol
    let m = model {
        let dt = (1.0f / 252.0f).C
        let! z = normal
    ...
    let vegaObs = watch.Observers |> List.tryFind (fun o -> o.Name = "__deriv_vol")
    Assert.True(vegaObs.IsSome)
    let vegaVals = Watcher.values "__deriv_vol" watch
    ...
    let vommaObs = watch.Observers |> List.tryFind (fun o -> o.Name = "__deriv2_vol")
...
let ``HyperDual rate convexity: bond d²V/dr² is positive`` () =
    ...
    let d2Obs = watch.Observers |> List.find (fun o -> o.Name = "__deriv2_rate")
...
let ``HyperDual vanna: cross-Greek d²V/dSdvol is non-zero`` () =
    ...
    let m = {
        ...
        Surfaces = Map.empty; Observers = []; NormalCount = 1; UniformCount = 0; BatchSize = 0
    }
    ...
    let _, watch = Simulation.foldWatch sim expanded Monthly
    // Cross-term __deriv2_spot_vol should exist
    let crossObs = expanded.Observers |> List.tryFind (fun o -> o.Name = "__deriv2_spot_vol")
    Assert.True(crossObs.IsSome, "Cross-term observer __deriv2_spot_vol should exist")
    // Vanna should be non-zero for a call option
    let crossVals = Watcher.values "__deriv2_spot_vol" watch
...
let ``Mixed-order: Dual spot + HyperDual vol produces delta but no gamma for spot`` () =
    ...
    use sim = Simulation.create CPU 5_000 252
    let _, watch = Simulation.foldWatch sim m Monthly
    // Should have __deriv_spot (delta) — 1st order for spot
    let deltaObs = watch.Observers |> List.tryFind (fun o -> o.Name = "__deriv_spot")
    Assert.True(deltaObs.IsSome, "Delta observer should exist")
    // Should have __deriv_vol (vega) — 1st order for vol
    let vegaObs = watch.Observers |> List.tryFind (fun o -> o.Name = "__deriv_vol")
    Assert.True(vegaObs.IsSome, "Vega observer should exist")
    // Should have __deriv2_vol (vomma) — 2nd order only for HyperDual vol
    let vommaObs = watch.Observers |> List.tryFind (fun o -> o.Name = "__deriv2_vol")
    Assert.True(vommaObs.IsSome, "Vomma observer should exist for HyperDual vol")
    // Should NOT have __deriv2_spot — spot is Dual, not HyperDual
    let gammaObs = watch.Observers |> List.tryFind (fun o -> o.Name = "__deriv2_spot")
...
let ``HyperDual 3-param diagonal: spot + rate + vol all have finite 2nd derivatives`` () =
    ...
    for name in ["__deriv2_spot"; "__deriv2_rate"; "__deriv2_vol"] do
    ...
    for name in ["__deriv_spot"; "__deriv_rate"; "__deriv_vol"] do
        let obs = watch.Observers |> List.tryFind (fun o -> o.Name = name)
        Assert.True(obs.IsSome, $"Observer {name} should exist")
