...
let private applyPayoff (payoff: PayoffSpec) (rate: Expr) (dt: Expr) (spot: Expr) : ModelCtx -> Expr = fun ctx ->
    let raw =
        if isNull payoff then spot
        else
...
let private toFloatArray (rf: RepeatedField<float32>) : float32[] =
    rf |> Seq.toArray
...
let private buildGBM (m: GBMModel) (observers: RepeatedField<string>) (batchValues: float32[] option) : Model =
    model {
        ...
        do! fun ctx -> for name in observers do observe name stock ctx
        return result
    }

let private buildGBMLocalVol (m: GBMLocalVolModel) (observers: RepeatedField<string>) (_batchValues: float32[] option) : Model =
    let vs = m.VolSurface
...
let private buildVasicek (m: VasicekModel) (observers: RepeatedField<string>) (_batchValues: float32[] option) : Model =
    model {
        ...
        do! fun ctx -> for name in observers do observe name rate ctx
        return rate
    }
...
let private buildMultiAssetHeston (m: MultiAssetHestonModel) (observers: RepeatedField<string>) (_batchValues: float32[] option) : Model =
    ...
    let assets =
        m.Assets
        |> Seq.map (fun a ->
            { Equity.HestonAsset.Spot = a.Spot.C; V0 = a.V0.C; Kappa = a.Kappa.C
              Theta = a.Theta.C; Xi = a.Xi.C; Rho = a.Rho })
        |> Seq.toArray
    model {
        let dt = (1.0f / float32 m.Steps).C
        let rate = m.Rate.C
...
let rec exprFromProto (node: ExprNode) : Expr =
    ...
    | ExprNode.ExprOneofCase.SurfaceAt ->
        SurfaceAt(node.SurfaceAt.SurfaceId, exprFromProto node.SurfaceAt.Index)
    | ExprNode.ExprOneofCase.BinSearch ->
        let bs = node.BinSearch
        BinSearch(bs.SurfaceId, bs.AxisOff, bs.AxisCnt, exprFromProto bs.Value)
    | ExprNode.ExprOneofCase.Dual ->
        Dual(node.Dual.Index, node.Dual.Value, node.Dual.Name)
    | ExprNode.ExprOneofCase.HyperDual ->
        HyperDual(node.HyperDual.Index, node.HyperDual.Value, node.HyperDual.Name)
    | _ -> failwith $"Unknown ExprNode case: {node.ExprCase}"

let private buildCustom (m: CustomModel) : Model =
    let accums =
        m.Accums
        |> Seq.map (fun a -> a.Id, { Init = exprFromProto a.Init; Body = exprFromProto a.Body })
        |> Map.ofSeq
    let surfaces =
        m.Surfaces
        |> Seq.map (fun s ->
            let data =
                ...
                | SurfaceDef.DataOneofCase.Curve1D ->
                    Curve1D(toFloatArray s.Curve1D.Values, s.Curve1D.Steps)
...
let buildModel (spec: ModelSpec) : Model =
    buildModelInner spec None

let buildBatchModel (spec: ModelSpec) (batchValues: float32[]) : Model =
    buildModelInner spec (Some batchValues)
