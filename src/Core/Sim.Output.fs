...
module Output =
    ...
    module Parquet =
        ...
        let writeFold (path: string) (values: float32[]) : unit =
            let schema = ParquetSchema(DataField<int>("scenario"), DataField<float32>("value"))
            ...
            rg.WriteColumnAsync(DataColumn(schema.DataFields.[0], scenarios))
            |> Async.AwaitTask
            |> Async.RunSynchronously

            rg.WriteColumnAsync(DataColumn(schema.DataFields.[1], values))
            |> Async.AwaitTask
            |> Async.RunSynchronously
        ...
        let writeScan (path: string) (data: float32[,]) : unit =
            ...
            rg.WriteColumnAsync(DataColumn(schema.DataFields.[0], stepCol))
            |> Async.AwaitTask
            |> Async.RunSynchronously
            ...
            for s in 0 .. sims - 1 do
                ...
                rg.WriteColumnAsync(DataColumn(schema.DataFields.[s + 1], col))
                |> Async.AwaitTask
                |> Async.RunSynchronously
    ...
    let writeFold (path: string) (format: OutputFormat) (values: float32[]) : unit =
        ...
        | OutputFormat.Parquet -> Parquet.writeFold path values
    ...
    let writeScan (path: string) (format: OutputFormat) (data: float32[,]) : unit =
        ...
        | OutputFormat.Parquet -> Parquet.writeScan path data
    ...
    let writeWatch (path: string) (format: OutputFormat) (finals: float32[]) (watch: WatchResult) : unit =
        ...
        | OutputFormat.Parquet -> Parquet.writeFold path finals
