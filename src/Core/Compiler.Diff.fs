...
module CompilerDiff =
    ...
    let rec collectDiffVars (expr: Expr) : Set<int> =
        ...
        | AccumRef _
        | Lookup1D _
        | BatchRef _ -> Set.empty
        | Floor a
        | Neg a
        | Exp a
        | Log a
        | Sqrt a
        | Abs a -> collectDiffVars a
        ...
        | Gte(a, b)
        | Lt(a, b)
        | Lte(a, b) -> Set.union (collectDiffVars a) (collectDiffVars b)
        | Select(c, t, f) ->
            collectDiffVars c
            |> Set.union (collectDiffVars t)
            |> Set.union (collectDiffVars f)
        | BinSearch(_, _, _, v) -> collectDiffVars v
        | Cond(cases, d) ->
            cases
            |> List.fold
                (fun acc (c, v) -> Set.union acc (Set.union (collectDiffVars c) (collectDiffVars v)))
                (collectDiffVars d)
    ...
    let rec collectHyperDualVars (expr: Expr) : Set<int> =
        ...
        | Gte(a, b)
        | Lt(a, b)
        | Lte(a, b) -> Set.union (collectHyperDualVars a) (collectHyperDualVars b)
        | Select(c, t, f) ->
            collectHyperDualVars c
            |> Set.union (collectHyperDualVars t)
            |> Set.union (collectHyperDualVars f)
        | BinSearch(_, _, _, v) -> collectHyperDualVars v
        | Cond(cases, d) ->
            cases
            |> List.fold
                (fun acc (c, v) -> Set.union acc (Set.union (collectHyperDualVars c) (collectHyperDualVars v)))
                (collectHyperDualVars d)
    ...
    let rec private collectDiffVarNames (expr: Expr) : Map<int, string> =
        match expr with
        | Dual(idx, _, name)
        | HyperDual(idx, _, name) -> Map.ofList [ idx, name ]
        | Const _
        | TimeIndex
        | Normal _
        | Uniform _
        ...
        | AccumRef _
        | Lookup1D _
        | BatchRef _ -> Map.empty
        | Floor a
        | Neg a
        | Exp a
        | Log a
        | Sqrt a
        | Abs a -> collectDiffVarNames a
        ...
        | Cond(cases, d) ->
            ...
            |> List.fold
                (fun acc (c, v) ->
                    let mc = collectDiffVarNames c
                    let mv = collectDiffVarNames v
                    Map.fold (fun a k v -> Map.add k v a) (Map.fold (fun a k v -> Map.add k v a) acc mc) mv)
                (collectDiffVarNames d)
    ...
    let collectModelDiffVarNames (model: Model) : Map<int, string> =
        ...
        let fromAccums =
            model.Accums
            |> Map.toSeq
            |> Seq.collect (fun (_, def) -> [ collectDiffVarNames def.Init; collectDiffVarNames def.Body ])
        ...
        Map.fold
            ...
            (Map.fold (fun acc k v -> Map.add k v acc) fromResult fromAccums)
            fromObs
    ...
    let collectModelDiffVars (model: Model) : int[] =
        ...
        let fromAccums =
            model.Accums
            |> Map.toSeq
            |> Seq.collect (fun (_, def) -> Set.union (collectDiffVars def.Init) (collectDiffVars def.Body))
            |> Set.ofSeq

        let fromObs = model.Observers |> List.map (fun o -> collectDiffVars o.Expr) |> Set.unionMany
        Set.unionMany [ fromResult; fromAccums; fromObs ] |> Set.toArray |> Array.sort
    ...
    let collectModelHyperDualVars (model: Model) : int[] =
        ...
        let fromAccums =
            model.Accums
            |> Map.toSeq
            |> Seq.collect (fun (_, def) -> Set.union (collectHyperDualVars def.Init) (collectHyperDualVars def.Body))
            |> Set.ofSeq
    ...
    let rec forwardDiff (expr: Expr) (wrt: int) (accumDerivId: int -> int) : Expr =
        ...
        | Mul(a, b) -> Add(Mul(forwardDiff a wrt accumDerivId, b), Mul(a, forwardDiff b wrt accumDerivId))

        // Quotient rule: d(a/b) = (da*b - a*db) / b^2
        | Div(a, b) ->
            Div(Sub(Mul(forwardDiff a wrt accumDerivId, b), Mul(a, forwardDiff b wrt accumDerivId)), Mul(b, b))
        ...
        | Select(cond, t, f) -> Select(cond, forwardDiff t wrt accumDerivId, forwardDiff f wrt accumDerivId)

        // Cond: differentiate values (conditions are piecewise constant)
        | Cond(cases, d) ->
            Cond(cases |> List.map (fun (c, v) -> (c, forwardDiff v wrt accumDerivId)), forwardDiff d wrt accumDerivId)
    ...
    let transformDual (model: Model) : Model * int[] =
        ...
        let maxAccumId =
            if model.Accums.IsEmpty then
                -1
            else
                model.Accums |> Map.toSeq |> Seq.map fst |> Seq.max

        let derivAccumBase = maxAccumId + 1

        // Map from (original accum ID, diffVar position) → derivative accum ID
        let derivAccumId (origId: int) (pos: int) : int = derivAccumBase + origId * n + pos
    ...
    let transformHyperDual (diagonal: bool) (model: Model) : Model * int[] =
        ...
        if diagonal then
            ...
            for pos in 0 .. n - 1 do
                ...
                for (origId, _) in sortedOrigAccums do
                    ...
                    let accumDerivIdFn (id: int) =
                        if id <= maxOrigId then
                            layer1Id id pos
                        else
                            layer2DiagId ((id - layer1Base) / n) pos
...

            {
                model1 with
                    Accums = newAccums
                    Observers = model1.Observers @ layer2Observers
            },
            diffVars
                    ...
                    for (origId, _) in sortedOrigAccums do
                        ...
                        let accumDerivIdFn (id: int) =
                            if id <= maxOrigId then
                                ...
                                layer2FullId ((id - layer1Base) / n) i j
    ...
    let transformHyperDualSelective (hyperVarIndices: int[]) (model1: Model) (originalModel: Model) : Model =
        ...
        if hyperVarIndices.Length = 0 then
            model1
        else

        let maxOrigId =
            if originalModel.Accums.IsEmpty then
                -1
            else
                originalModel.Accums |> Map.toSeq |> Seq.map fst |> Seq.max
    ...
    let rec partialDiffAccumRef (expr: Expr) (targetId: int) : Expr =
        ...
        | Mul(a, b) -> Add(Mul(partialDiffAccumRef a targetId, b), Mul(a, partialDiffAccumRef b targetId))
        | Div(a, b) ->
            Div(Sub(Mul(partialDiffAccumRef a targetId, b), Mul(a, partialDiffAccumRef b targetId)), Mul(b, b))
        ...
        | Abs a -> Mul(Select(Gt(a, Const 0.0f), Const 1.0f, Const -1.0f), partialDiffAccumRef a targetId)
        ...
        | Lt _
        | Lte _ -> Const 0.0f
        | Select(c, t, f) -> Select(c, partialDiffAccumRef t targetId, partialDiffAccumRef f targetId)
        | Cond(cases, d) ->
            Cond(cases |> List.map (fun (c, v) -> (c, partialDiffAccumRef v targetId)), partialDiffAccumRef d targetId)
    ...
    let rec partialDiffDiffVar (expr: Expr) (targetIdx: int) : Expr =
        match expr with
        | Dual(idx, _, _)
        | HyperDual(idx, _, _) -> if idx = targetIdx then Const 1.0f else Const 0.0f
        | AccumRef _
        | Const _
        | TimeIndex
        | Normal _
        ...
        | Mul(a, b) -> Add(Mul(partialDiffDiffVar a targetIdx, b), Mul(a, partialDiffDiffVar b targetIdx))
        | Div(a, b) ->
            Div(Sub(Mul(partialDiffDiffVar a targetIdx, b), Mul(a, partialDiffDiffVar b targetIdx)), Mul(b, b))
        ...
        | Abs a -> Mul(Select(Gt(a, Const 0.0f), Const 1.0f, Const -1.0f), partialDiffDiffVar a targetIdx)
        ...
        | Lt _
        | Lte _ -> Const 0.0f
        | Select(c, t, f) -> Select(c, partialDiffDiffVar t targetIdx, partialDiffDiffVar f targetIdx)
        | Cond(cases, d) ->
            Cond(cases |> List.map (fun (c, v) -> (c, partialDiffDiffVar v targetIdx)), partialDiffDiffVar d targetIdx)
    ...
    let hasDiffVars (model: Model) : bool = (collectModelDiffVars model).Length > 0
