...
module Watcher =
    ...
    let numObs (interval: int) (steps: int) : int = (steps + interval - 1) / interval
    ...
    let terminals (name: string) (wr: WatchResult) : float32[] =
        ...
        Array.init wr.NumPaths (fun path -> wr.Buffer.[(spec.SlotIndex * wr.NumObs + lastObs) * wr.NumPaths + path])

    let sliceObs (name: string) (obsIdx: int) (wr: WatchResult) : float32[] =
        let spec = wr.Observers |> List.find (fun o -> o.Name = name)
        Array.init wr.NumPaths (fun path -> wr.Buffer.[(spec.SlotIndex * wr.NumObs + obsIdx) * wr.NumPaths + path])
