...
module CompilerCodegen =
    ...
    let dedent (s: string) =
        ...
        if nonEmptyLines.Length = 0 then s
        else
            let minIndent =
                nonEmptyLines
    ...
    let emitHelpers (_layout: SurfaceLayout) =
        [ csHash; csToFloat; csBoxMuller; csFindBin ]
        |> String.concat "\n"
    ...
    let emitAccumDecls (sb: Text.StringBuilder) (layout: SurfaceLayout) (sortedAccums: (int * AccumDef) list) =
        for (id, def) in sortedAccums do
            sb.AppendLine(sprintf "        float accum_%d = %s;" id (CompilerCommon.emitExpr layout def.Init)) |> ignore
    ...
    let emitNormals (sb: Text.StringBuilder) (model: Model) (seedVar: string) =
        for n in 0 .. model.NormalCount - 1 do
            ...
            sb.AppendLine(sprintf "                ToFloat(Hash(%s * 6364136 + t * 2 * normalCount + %d))," seedVar (n * 2)) |> ignore
            sb.AppendLine(sprintf "                ToFloat(Hash(%s * 6364136 + t * 2 * normalCount + %d)));" seedVar (n * 2 + 1)) |> ignore
    ...
    let emitObserverRecording (sb: Text.StringBuilder) (model: Model) (layout: SurfaceLayout) (bufferExpr: string) (totalExpr: string) =
        if not model.Observers.IsEmpty then
            sb.AppendLine("            if ((t + 1) % interval == 0 || t == steps - 1)") |> ignore
            sb.AppendLine("            {") |> ignore
            sb.AppendLine("                int slot = (t + 1) / interval - 1;") |> ignore
            sb.AppendLine("                if ((t + 1) % interval != 0) slot = numObs - 1;") |> ignore
            sb.AppendLine("                else slot = Math.Min(slot, numObs - 1);") |> ignore
            for obs in model.Observers do
                sb.AppendLine(sprintf "                %s[(%d * numObs + slot) * %s + idx] = %s;" bufferExpr obs.SlotIndex totalExpr (CompilerCommon.emitExpr layout obs.Expr)) |> ignore
            sb.AppendLine("            }") |> ignore
