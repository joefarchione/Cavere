...
type ObserverSpec = { Name: string; Expr: Expr; SlotIndex: int }
...
type ModelBuilder() =
    ...
    member _.Bind(f: ModelCtx -> 'T, g: 'T -> ModelCtx -> 'R) : ModelCtx -> 'R =
        fun ctx -> (g (f ctx)) ctx
    ...
    member _.Delay(f: unit -> ModelCtx -> 'T) : ModelCtx -> 'T =
        fun ctx -> f () ctx
...
module ModelDsl =
    ...
    let normal : ModelCtx -> Expr = fun ctx ->
        let id = ctx.NextNormalId
        ctx.NextNormalId <- id + 1
        Normal id

    let uniform : ModelCtx -> Expr = fun ctx ->
    ...
    let surface1d (values: float32[]) (steps: int) : ModelCtx -> int = fun ctx ->
        let id = ctx.NextSurfaceId
        ctx.NextSurfaceId <- id + 1
        ctx.Surfaces <- ctx.Surfaces |> Map.add id (Curve1D(values, steps))
        id

    let surface2d (times: float32[]) (spots: float32[]) (vols: float32[,]) (steps: int) : ModelCtx -> int = fun ctx ->
        let id = ctx.NextSurfaceId
        ctx.NextSurfaceId <- id + 1
        let flat =
            Array.init (Array2D.length1 vols * Array2D.length2 vols)
                (fun i -> vols.[i / Array2D.length2 vols, i % Array2D.length2 vols])
        ctx.Surfaces <- ctx.Surfaces |> Map.add id (Grid2D(flat, times, spots, steps))
        id

    let scheduleDt (sched: Schedule) : ModelCtx -> Expr = fun ctx ->
        let id = ctx.NextSurfaceId
        ctx.NextSurfaceId <- id + 1
        ctx.Surfaces <- ctx.Surfaces |> Map.add id (Curve1D(sched.Dt, sched.Steps))
        Lookup1D id

    let scheduleT (sched: Schedule) : ModelCtx -> Expr = fun ctx ->
        let id = ctx.NextSurfaceId
        ctx.NextSurfaceId <- id + 1
        ctx.Surfaces <- ctx.Surfaces |> Map.add id (Curve1D(sched.T, sched.Steps))
        Lookup1D id

    let schedule (sched: Schedule) : ModelCtx -> Expr * Expr = fun ctx ->
        let dt = scheduleDt sched ctx
        let t = scheduleT sched ctx
        dt, t

    let batchInput (values: float32[]) : ModelCtx -> Expr = fun ctx ->
        let id = ctx.NextSurfaceId
        ctx.NextSurfaceId <- id + 1
    ...
    let observe (name: string) (expr: Expr) : ModelCtx -> unit = fun ctx ->
        let slot = ctx.Observers.Length
        ctx.Observers <- { Name = name; Expr = expr; SlotIndex = slot } :: ctx.Observers
    ...
    let interp1d (surfaceId: int) (t: Expr) : ModelCtx -> Expr = fun ctx ->
    ...
    let private binSearchThreshold = 10
    ...
    let interp2d (surfaceId: int) (t: Expr) (s: Expr) : ModelCtx -> Expr = fun ctx ->
        match ctx.Surfaces.[surfaceId] with
        | Grid2D(values, timeAxis, spotAxis, steps) ->
            ...
            let tFrac =
                Expr.select (tSpan .> 0.0f)
                    (Expr.clip 0.0f.C 1.0f.C ((tNorm - tLoVal) / tSpan))
                    0.0f.C
            ...
            let sFrac =
                Expr.select (sSpan .> 0.0f)
                    (Expr.clip 0.0f.C 1.0f.C ((s - sLoVal) / sSpan))
                    0.0f.C
    ...
    let correlatedNormals (correlation: float32[,]) : ModelCtx -> Expr[] = fun ctx ->
