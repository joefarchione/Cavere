...
module Riders =
    ...
    module RiderCharge =
        let onBenefitBase (rate: float32) : RiderCharge =
            { AnnualRate = rate; ChargeBase = BenefitBase; MaxCharge = None }

        let onAccountValue (rate: float32) : RiderCharge =
            { AnnualRate = rate; ChargeBase = AccountValue; MaxCharge = None }

        let onGreaterOf (rate: float32) : RiderCharge =
            { AnnualRate = rate; ChargeBase = GreaterOfAVOrBB; MaxCharge = None }

        let withMaxCharge (maxRate: float32) (charge: RiderCharge) : RiderCharge =
            { charge with MaxCharge = Some maxRate }
    ...
    module GMAB =
        ...
        let withStepUp (stepUp: StepUpProvision) (gmab: GMAB) : GMAB =
            { gmab with StepUp = stepUp }

        /// Calculate GMAB benefit base.
        let benefitBase (premium: Expr) (accountValue: Expr) (rollUpValue: Expr)
                        (highWaterMark: Expr) (growth: BenefitBaseGrowth) : Expr =
            match growth with
            | NoGrowth -> premium
        ...
        let payout (benefitBase: Expr) (accountValue: Expr) : Expr =
            Expr.max (benefitBase - accountValue) 0.0f.C
    ...
    module GMIB =
        ...
        let withRollUp (rollUpRate: float32) (waitingYears: int)
                       (payoutRate: float32) (charge: RiderCharge) : GMIB =
            { WaitingPeriod = waitingYears
        ...
        let withAgeBasedPayouts (rates: (int * float32) list) (gmib: GMIB) : GMIB =
            { gmib with PayoutRateByAge = Some rates }

        /// Guaranteed annual income = benefitBase * payoutRate.
        let guaranteedIncome (benefitBase: Expr) (payoutRate: float32) : Expr =
            benefitBase * payoutRate.C
    ...
    module GMWB =
        ...
        let withRollUp (withdrawalRate: float32) (rollUpRate: float32)
                       (rollUpPeriod: int) (charge: RiderCharge) : GMWB =
        ...
        let withExcessPenalty (penalty: float32) (gmwb: GMWB) : GMWB =
            { gmwb with ExcessWithdrawalPenalty = penalty }

        /// Guaranteed withdrawal amount.
        let guaranteedWithdrawal (benefitBase: Expr) (withdrawalRate: float32) : Expr =
            benefitBase * withdrawalRate.C
        ...
        let benefitBaseReductionForExcess (benefitBase: Expr) (accountValue: Expr)
                                          (excessWithdrawal: Expr) : Expr =
    ...
    module GLWB =
        ...
        let create (singleRate: float32) (jointRate: float32)
                   (rollUpRate: float32) (waitingYears: int) (charge: RiderCharge) : GLWB =
        ...
        let withAgeBasedRates (rates: (int * float32) list) (glwb: GLWB) : GLWB =
            { glwb with WithdrawalRatesByAge = rates }

        /// Add deferral bonus.
        let withDeferralBonus (ratePerYear: float32) (maxYears: int) (glwb: GLWB) : GLWB =
            { glwb with BonusProvisions = Some (DeferralBonus (ratePerYear, maxYears)) }

        /// Lifetime withdrawal amount (continues even if AV = 0).
        let lifetimeWithdrawal (benefitBase: Expr) (withdrawalRate: float32) : Expr =
            benefitBase * withdrawalRate.C
    ...
    module GMDB =
        ...
        let withMaxAge (age: int) (gmdb: GMDB) : GMDB =
            { gmdb with MaxAge = Some age }

        /// Calculate GMDB benefit.
        let deathBenefit (accountValue: Expr) (premium: Expr) (rollUpValue: Expr)
                         (highWaterMark: Expr) (gmdbType: GMDBType) : Expr =
            match gmdbType with
            | ReturnOfPremium ->
                Expr.max accountValue premium
            | RollUp _ ->
                Expr.max accountValue rollUpValue
            | HighestAnniversaryValue ->
                Expr.max accountValue highWaterMark
            | RatchetAndRollUp _ ->
                Expr.max accountValue (Expr.max rollUpValue highWaterMark)
            | EstatePlus pct ->
    ...
    module BenefitBase =
        ...
        let rollUp (rate: float32) (dt: Expr) (init: Expr) : ModelCtx -> Expr = fun ctx ->
            evolve init (fun bb -> bb * Expr.exp (rate.C * dt)) ctx

        /// High water mark accumulator (tracks highest AV on anniversaries).
        let highWaterMark (accountValue: Expr) (init: Expr) : ModelCtx -> Expr = fun ctx ->
            evolve init (fun hwm -> Expr.max hwm accountValue) ctx

        /// Combined roll-up with high water mark.
        let rollUpWithHWM (rate: float32) (dt: Expr) (accountValue: Expr) (init: Expr) : ModelCtx -> Expr = fun ctx ->
    ...
    module Charge =
        ...
        let chargeBase (accountValue: Expr) (benefitBase: Expr) (premium: Expr)
                       (base': ChargeBase) : Expr =
            match base' with
        ...
        let applyCharge (accountValue: Expr) (chargeAmount: Expr) : Expr =
            Expr.max (accountValue - chargeAmount) 0.0f.C
    ...
    module RiderPackage =
        let empty : RiderPackage =
            { GMAB = None; GMIB = None; GMWB = None; GLWB = None; GMDB = None }

        let withGMAB (gmab: GMAB) (pkg: RiderPackage) : RiderPackage =
            { pkg with GMAB = Some gmab }

        let withGMIB (gmib: GMIB) (pkg: RiderPackage) : RiderPackage =
            { pkg with GMIB = Some gmib }

        let withGMWB (gmwb: GMWB) (pkg: RiderPackage) : RiderPackage =
            { pkg with GMWB = Some gmwb }

        let withGLWB (glwb: GLWB) (pkg: RiderPackage) : RiderPackage =
            { pkg with GLWB = Some glwb }

        let withGMDB (gmdb: GMDB) (pkg: RiderPackage) : RiderPackage =
            { pkg with GMDB = Some gmdb }

        /// Total annual rider charges.
        let totalAnnualCharge (pkg: RiderPackage) : float32 =
            let getRate = function Some r -> r.Charge.AnnualRate | None -> 0.0f
