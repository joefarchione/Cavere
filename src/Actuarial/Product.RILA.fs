...
module RILA =
    ...
    module ProtectionType =
        ...
        let buffer (level: float32) : ProtectionType = Buffer { BufferLevel = level }

        /// Floor protection.
        let floor (level: float32) : ProtectionType = Floor { FloorLevel = level }

        /// Dual direction with buffer and participation.
        let dualDirection (buffer: float32) (participation: float32) : ProtectionType =
            DualDirection {
                DownsideBuffer = buffer
                UpsideParticipation = participation
            }
    ...
    module RILAProduct =
        ...
        let create (name: string) (strategy: RILAStrategy) (cdsc: Common.CDSCSchedule) : RILAProduct = {
            Name = name
            Strategies = [ strategy ]
            CDSCSchedule = cdsc
        ...
        /// Total rider charges.
        let totalRiderCharge (product: RILAProduct) : float32 = Riders.RiderPackage.totalAnnualCharge product.Riders
    ...
    module Crediting =
        ...
        let applyBuffer (indexReturn: Expr) (bufferLevel: float32) : Expr =
            ...
            Expr.select
                (indexReturn .>= 0.0f)
                indexReturn
                (Expr.select (indexReturn .>= (-bufferLevel).C) 0.0f.C (indexReturn + bufferLevel.C))

        /// Apply floor protection to index return.
        let applyFloor (indexReturn: Expr) (floorLevel: float32) : Expr = Expr.max floorLevel.C indexReturn

        /// Apply cap to upside.
        let applyCap (indexReturn: Expr) (cap: float32) : Expr = Expr.min cap.C indexReturn

        /// Apply participation rate.
        let applyParticipation (indexReturn: Expr) (rate: float32) : Expr = indexReturn * rate.C
        ...
        let accumulateRILA (creditedReturn: Expr) (accountValue: Expr) : Expr = accountValue * (1.0f.C + creditedReturn)
