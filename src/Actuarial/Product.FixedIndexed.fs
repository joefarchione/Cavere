...
module FixedIndexed =
    ...
    module CreditingStrategy =
        ...
        let pointToPoint (cap: float32) (participation: float32) : CreditingStrategy =
            PointToPoint { Cap = cap; Floor = 0.0f; Participation = participation; Spread = 0.0f }
        ...
        let pointToPointWithSpread (spread: float32) (participation: float32) : CreditingStrategy =
            PointToPoint { Cap = 1.0f; Floor = 0.0f; Participation = participation; Spread = spread }

        /// Monthly cap strategy.
        let monthlyCap (cap: float32) : CreditingStrategy =
            MonthlyCap { Cap = cap; Floor = 0.0f }

        /// Monthly average with participation.
        let monthlyAverage (cap: float32) (participation: float32) : CreditingStrategy =
            MonthlyAverage { Cap = cap; Participation = participation }

        /// Performance trigger strategy.
        let performanceTrigger (rate: float32) : CreditingStrategy =
            PerformanceTrigger { TriggerRate = rate }

        /// Fixed account (guaranteed rate).
        let fixedAccount (rate: float32) : CreditingStrategy =
            FixedAccount rate
    ...
    module IndexAllocation =
        let create (index: IndexType) (strategy: CreditingStrategy) (pct: float32) : IndexAllocation =
            { Index = index; Strategy = strategy; Allocation = pct }
    ...
    module FIAProduct =
        ...
        let create (name: string) (allocation: IndexAllocation)
                   (cdsc: Common.CDSCSchedule) : FIAProduct =
            {
                Name = name
        ...
        let createMultiIndex (name: string) (allocations: IndexAllocation list)
                             (cdsc: Common.CDSCSchedule) : FIAProduct =
            {
                Name = name
        ...
        let withBonus (bonusPct: float32) (vestingYears: int) (product: FIAProduct) : FIAProduct =
            { product with BonusPercent = Some bonusPct; BonusVestingYears = vestingYears }

        /// Add MVA to FIA.
        let withMVA (mva: Common.MVAParams) (product: FIAProduct) : FIAProduct =
        ...
        let withMinGuarantee (pct: float32) (product: FIAProduct) : FIAProduct =
            { product with MinGuaranteedValue = pct }
        ...
        /// Total rider charges.
        let totalRiderCharge (product: FIAProduct) : float32 =
    ...
    module Crediting =
        ...
        let accumulateWithCredit (creditedRate: Expr) (dt: Expr) (init: Expr) : ModelCtx -> Expr = fun ctx ->
            evolve init (fun av -> av * (1.0f.C + creditedRate * dt)) ctx

        /// Index return from start to end values.
        let indexReturn (startValue: Expr) (endValue: Expr) : Expr =
    ...
    module Guarantees =
        ...
        let mgav (premium: Expr) (guaranteeRate: Expr) (dt: Expr) : ModelCtx -> Expr = fun ctx ->
            evolve premium (fun mgav -> mgav * Expr.exp (guaranteeRate * dt)) ctx

        /// Account value floored at MGAV.
        let avWithMgavFloor (accountValue: Expr) (mgav: Expr) : Expr =
            Expr.max accountValue mgav

        /// Premium bonus applied to account value.
        let applyBonus (premium: Expr) (bonusPercent: float32) : Expr =
            premium * (1.0f + bonusPercent).C
