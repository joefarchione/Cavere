...
module Surrender =
    ...
    let loadSurrenderTable (ratesByDuration: float32[]) : ModelCtx -> int = fun ctx ->
    ...
    let surrenderRate (surfaceId: int) : Expr =
        Lookup1D surfaceId

    /// Surrender rate lookup with interpolation for fractional durations.
    let surrenderRateInterp (surfaceId: int) (duration: Expr) : ModelCtx -> Expr = fun ctx ->
        interp1d surfaceId duration ctx
    ...
    let persistency (surrenderRate: Expr) : ModelCtx -> Expr = fun ctx ->
        survivalProbAnnual surrenderRate ctx

    /// Cumulative persistency with sub-annual time steps.
    let persistencySubannual (annualSurrenderRate: Expr) (dt: Expr) : ModelCtx -> Expr = fun ctx ->
    ...
    let loadSurrenderChargeTable (chargesByDuration: float32[]) : ModelCtx -> int = fun ctx ->
        let id = ctx.NextSurfaceId
        ctx.NextSurfaceId <- id + 1
        ctx.Surfaces <- ctx.Surfaces |> Map.add id (Curve1D(chargesByDuration, chargesByDuration.Length))
        id

    /// Surrender charge lookup by policy duration.
    let surrenderCharge (surfaceId: int) : Expr =
        Lookup1D surfaceId
    ...
    let cumulativeSurrenderPayouts (csv: Expr) (surrenderRate: Expr) (persistency: Expr) (df: Expr) : ModelCtx -> Expr = fun ctx ->
