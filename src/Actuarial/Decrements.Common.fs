...
module Common =
    ...
    let rateByTime (surfaceId: int) (t: Expr) : ModelCtx -> Expr = fun ctx ->
        interp1d surfaceId t ctx

    /// Decrement rate from a 2D surface (e.g., by duration and attained age)
    let rateByDurationAndAge (surfaceId: int) (age: Expr) : ModelCtx -> Expr = fun ctx ->
        interp2d surfaceId TimeIndex age ctx
    ...
    let survivalProb (rate: Expr) (dt: Expr) : ModelCtx -> Expr = fun ctx ->
        evolve 1.0f.C (fun px -> px * (1.0f.C - rate * dt)) ctx
    ...
    let survivalProbAnnual (annualRate: Expr) : ModelCtx -> Expr = fun ctx ->
        evolve 1.0f.C (fun px -> px * (1.0f.C - annualRate)) ctx

    /// Cumulative survival using exponential model: exp(-rate * dt) each step.
    /// More accurate for continuous-time decrements.
    let survivalProbContinuous (rate: Expr) (dt: Expr) : ModelCtx -> Expr = fun ctx ->
        evolve 1.0f.C (fun px -> px * Expr.exp (-rate * dt)) ctx
    ...
    let multiDecrementSurvival (decrements: (Expr * Expr) list) : ModelCtx -> Expr = fun ctx ->
        let combinedRate =
            decrements
            |> List.map (fun (rate, dt) -> rate * dt)
            |> List.reduce (+)
    ...
    let weightedByInforce (value: Expr) (survivalProb: Expr) : Expr =
        value * survivalProb
