...
module Mortality =
    ...
    let attainedAge (issueAge: Expr) : Expr =
        issueAge + TimeIndex

    /// Attained age with fractional years using year fraction.
    let attainedAgeFractional (issueAge: Expr) (yearFrac: Expr) : Expr =
        issueAge + yearFrac
    ...
    let loadMortalityTable (qxByAge: float32[]) : ModelCtx -> int = fun ctx ->
    ...
    let qx (surfaceId: int) (attainedAge: Expr) : ModelCtx -> Expr = fun ctx ->
    ...
    let qxFromIssueAge (surfaceId: int) (issueAge: Expr) : ModelCtx -> Expr = fun ctx ->
        interp1d surfaceId (issueAge + TimeIndex) ctx
    ...
    let qxFirstToDie (qxOwner: Expr) (qxSpouse: Expr) : Expr =
        qxOwner + qxSpouse - qxOwner * qxSpouse

    /// Joint mortality rate for last-to-die.
    /// qxy = qx * qy (probability both die in the period)
    let qxLastToDie (qxOwner: Expr) (qxSpouse: Expr) : Expr =
        qxOwner * qxSpouse
    ...
    let qxJointFromIssueAges (surfaceId: int) (jointType: JointMortalityType)
                             (ownerIssueAge: Expr) (spouseIssueAge: Expr) : ModelCtx -> Expr = fun ctx ->
    ...
    let qxJointSeparateTables (ownerSurfaceId: int) (spouseSurfaceId: int)
    ...
    let survivalProb (qx: Expr) : ModelCtx -> Expr = fun ctx ->
        survivalProbAnnual qx ctx

    /// Cumulative survival with sub-annual time steps.
    /// Adjusts annual qx to per-period rate.
    let survivalProbSubannual (annualQx: Expr) (dt: Expr) : ModelCtx -> Expr = fun ctx ->
    ...
    let survivalProbContinuous (qx: Expr) (dt: Expr) : ModelCtx -> Expr = fun ctx ->
    ...
    let survivalProbFirstToDie (pxOwner: Expr) (pxSpouse: Expr) : Expr =
        pxOwner * pxSpouse

    /// Joint survival probability for last-to-die.
    /// pxy = px + py - px * py (at least one must survive)
    let survivalProbLastToDie (pxOwner: Expr) (pxSpouse: Expr) : Expr =
        pxOwner + pxSpouse - pxOwner * pxSpouse

    /// Cumulative joint survival using mortality rates.
    /// Tracks survival of both lives separately, then combines.
    let survivalProbJoint (jointType: JointMortalityType)
                          (qxOwner: Expr) (qxSpouse: Expr) : ModelCtx -> Expr = fun ctx ->
    ...
    let survivalProbJointFromIssueAges (surfaceId: int) (jointType: JointMortalityType)
                                       (ownerIssueAge: Expr) (spouseIssueAge: Expr) : ModelCtx -> Expr = fun ctx ->
        let qxOwner = qxFromIssueAge surfaceId ownerIssueAge ctx
        let qxSpouse = qxFromIssueAge surfaceId spouseIssueAge ctx
    ...
    let cumulativeDeathClaims (deathBenefit: Expr) (qx: Expr) (survivalProb: Expr) (df: Expr) : ModelCtx -> Expr = fun ctx ->
    ...
    let expectedDeathBenefitJoint (deathBenefit: Expr) (qxJoint: Expr)
                                  (survivalProbJoint: Expr) (df: Expr) : Expr =
