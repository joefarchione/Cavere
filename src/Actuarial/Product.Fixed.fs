...
module Fixed =
    ...
    module AccountValue =
        ...
        let accumulateFixed (rate: Expr) (dt: Expr) (init: Expr) : ModelCtx -> Expr =
            fun ctx -> evolve init (fun av -> av * (1.0f.C + rate * dt)) ctx

        /// Compound interest accumulation.
        let accumulateCompound (rate: Expr) (dt: Expr) (init: Expr) : ModelCtx -> Expr =
            fun ctx -> evolve init (fun av -> av * Expr.exp (rate * dt)) ctx

        /// Account value with fee deduction.
        let accumulateWithFees (rate: Expr) (feeRate: Expr) (dt: Expr) (init: Expr) : ModelCtx -> Expr =
            fun ctx ->
