...
module Variable =
    ...
    module SubAccount =
        ...
        let equity (name: string) (expense: float32) (vol: float32) : SubAccount = create name Equity expense vol

        /// Common bond sub-account.
        let bond (name: string) (expense: float32) (vol: float32) : SubAccount = create name FixedIncome expense vol
    ...
    module VAProduct =
        ...
        let create (name: string) (allocations: SubAccountAllocation list) (fees: Common.FeeStructure) : VAProduct = {
            Name = name
        ...
        let withCDSC (cdsc: Common.CDSCSchedule) (product: VAProduct) : VAProduct = {
            product with
                CDSCSchedule = Some cdsc
        }

        /// Add rider package.
        let withRiders (riders: Riders.RiderPackage) (product: VAProduct) : VAProduct = { product with Riders = riders }

        /// Add GMAB rider.
        let withGMAB (gmab: Riders.GMAB) (product: VAProduct) : VAProduct = {
            product with
                Riders = Riders.RiderPackage.withGMAB gmab product.Riders
        }

        /// Add GMIB rider.
        let withGMIB (gmib: Riders.GMIB) (product: VAProduct) : VAProduct = {
            product with
                Riders = Riders.RiderPackage.withGMIB gmib product.Riders
        }

        /// Add GMWB rider.
        let withGMWB (gmwb: Riders.GMWB) (product: VAProduct) : VAProduct = {
            product with
                Riders = Riders.RiderPackage.withGMWB gmwb product.Riders
        }
        ...
        /// Total rider charges.
        let totalRiderCharge (product: VAProduct) : float32 = Riders.RiderPackage.totalAnnualCharge product.Riders
    ...
    module AccountValue =
        ...
        let weightedReturn (returns: (float32 * Expr) list) : Expr =
            returns |> List.map (fun (weight, ret) -> weight.C * ret) |> List.reduce (+)
        ...
        let accumulateWithFees (return': Expr) (fees: Expr) (dt: Expr) (init: Expr) : ModelCtx -> Expr =
            fun ctx -> evolve init (fun av -> grow av return' fees dt) ctx
