...
module Rates =
        ...
        let logDfs =
            ...
            |> Array.mapi (fun i t ->
                if i = 0 then 0.0f
                    ...
                    [| 1 .. i |]
                    |> Array.sumBy (fun j ->
                        -forwardRates.[j - 1] * (tenors.[j] - tenors.[j - 1])))
        let dt = 1.0f / float32 steps
        Array.init steps (fun i ->
    ...
    let curve (surfId: int) : ModelCtx -> Expr = fun ctx ->
        interp1d surfId TimeIndex ctx

    // ── Stochastic rate models ──

    let vasicek (kappa: Expr) (theta: Expr) (sigma: Expr) (r0: float32) (dt: Expr) : ModelCtx -> Expr = fun ctx ->
        let z = normal ctx
        evolve r0.C (fun r ->
            r + kappa * (theta - r) * dt + sigma * Expr.sqrt dt * z) ctx

    let cir (kappa: Expr) (theta: Expr) (sigma: Expr) (r0: float32) (dt: Expr) : ModelCtx -> Expr = fun ctx ->
        let z = normal ctx
        evolve r0.C (fun r ->
            let rp = Expr.max r 0.0f.C
            r + kappa * (theta - rp) * dt + sigma * Expr.sqrt (rp * dt) * z) ctx

    let cirpp
        (kappa: Expr) (theta: Expr) (sigma: Expr) (x0: float32)
        (shiftSurfId: int) (dt: Expr)
        : ModelCtx -> Expr = fun ctx ->
        let z = normal ctx
        let x = evolve x0.C (fun x ->
            let xp = Expr.max x 0.0f.C
            x + kappa * (theta - xp) * dt + sigma * Expr.sqrt (xp * dt) * z) ctx
