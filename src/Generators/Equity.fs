...
module Equity =
    ...
    let gbm (z: Expr) (rate: Expr) (vol: Expr) (spot: Expr) (dt: Expr) : ModelCtx -> Expr =
        fun ctx ->
            evolve
                spot
                (fun price ->
    ...
    let gbmLocalVol (z: Expr) (surfId: int) (rate: Expr) (spot: Expr) (dt: Expr) : ModelCtx -> Expr =
        fun ctx ->
            evolve
                spot
                (fun price ->
    ...
    let heston
        ...
        fun ctx ->
            ...
            let v =
                evolve
                    ...
                    (fun v ->
                        ...
                        v + kappa * (theta - vp) * dt + xi * Expr.sqrt vp * dw2)
                    ctx
            ...
            evolve
                ...
                (fun s ->
                    let vp = Expr.max v 0.0f.C
                    s * Expr.exp ((rate - 0.5f * vp) * dt + Expr.sqrt vp * dw1))
                ctx
            ...
            Array.init n (fun i ->
                ...
                let v =
                    evolve
                        ...
                        (fun v ->
                            let vp = Expr.max v 0.0f.C
                            v + a.Kappa * (a.Theta - vp) * dt + a.Xi * Expr.sqrt vp * dw2)
                        ctx
                ...
                evolve
                    ...
                    (fun s ->
                        let vp = Expr.max v 0.0f.C
                        s * Expr.exp ((rate - 0.5f.C * vp) * dt + Expr.sqrt vp * dw1))
                    ctx)
